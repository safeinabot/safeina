...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("**safeina1Bot") and event.query.user_id == bot.uid:
            buttons = [
                (
                    custom.Button.inline("Stats", data="stats"),
                    Button.url("Repo", "https://github.com/safeinabot/safeina/tree/main/SM"),
                )
            ]
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid: 
            ...
            reply_pop_up_alert = "⌔∮ عليك الحصول على بوت سفينه للحصول عليه اذهب الى @safeina1. "
